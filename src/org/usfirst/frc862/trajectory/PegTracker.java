...
public class PegTracker {
    ...
    public static class TrackReport {
        ...
        public TrackReport(PegTrack track) {
    ...
    List<PegTrack> mCurrentTracks = new ArrayList<>();
    int mNextId = 0;
    ...
    public void update(double timestamp, List<Translation2d> field_to_goals) {
        ...
        for (Iterator<PegTrack> it = mCurrentTracks.iterator(); it.hasNext();) {
            PegTrack track = it.next();
            if (!track.isAlive()) {
                it.remove();
            }
        }
        ...
        if (mCurrentTracks.isEmpty()) {
            for (Translation2d target : field_to_goals) {
                mCurrentTracks.add(PegTrack.makeNewTrack(timestamp, target, mNextId));
                ++mNextId;
            }
        }
    }
